...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, /*style.FrameRounding*/ 3.f);
    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    if (window->DC.ItemFlags & ImGuiItemFlags_MixedValue)
    {
        // Undocumented tristate/mixed/indeterminate checkbox (#2644)
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)));
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, /*style.FrameRounding*/ 3.f);
    }
    else if (*v)
    {
        //const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad*2.0f);
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), check_col, /*style.FrameRounding*/ 3.f);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    bool popup_open = IsPopupOpen(id, flags);

    const ImU32 frame_col = GetColorU32(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    RenderNavHighlight(frame_bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), frame_col, /*style.FrameRounding*/ 3.f, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, /*style.FrameRounding*/ 3.f, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, /*style.FrameRounding*/ 3.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    frame_bb.Max.y += (text_size.y / 1.5f) - 3.0f;
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    frame_bb.Min.y += (text_size.y / 1.5f) - 3.0f;
    ...
    frame_bb.Min.y += (text_size.y / 1.5f) - 3.0f;
    frame_bb.Max.y += (text_size.y / 1.5f) - 3.0f;
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), frame_col, true, g.Style.FrameRounding);
    ...
    //frame_bb.Min += ImVec2(0.f, 3.f);
    //frame_bb.Max -= ImVec2(0.f, 3.f);

    if (data_type == ImGuiDataType_Float)
        grab_bb.Max.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * (*(float*)p_data * 100.f / (*(float*)p_max)) / 100.f;

    if (data_type == ImGuiDataType_S32) // int
        grab_bb.Max.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * (float)((float)(*(int*)p_data) * 100.f / (float)(*(int*)p_max)) / 100.f;

    float pc = 0.0f;

    if (data_type == ImGuiDataType_S32)
        pc = (float)((float)*(int*)p_data) * 100.f / (float)(*(int*)p_max);

    if (data_type == ImGuiDataType_Float)
        pc = (float)((float)(*(float*)p_data) * 100.f / (float)(*(float*)p_max));

    // Render grab
    //if (grab_bb.Max.x > grab_bb.Min.x)
    if(pc >= 1.f)
        window->DrawList->AddRectFilled(/*grab_bb.Min*/ ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(grab_bb.Max.x, grab_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);

    //frame_bb.Min -= ImVec2(0.f, 3.f);
    //frame_bb.Max += ImVec2(0.f, 3.f);

    if (IsItemHovered())
        RenderText(grab_bb.Max - text_size + ImVec2(0.f, 8.f) , value_buf, value_buf_end);
        //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x -0.5f, frame_bb.Min.y  /*style.FramePadding.y*/ - ((text_size.y / 1.5f) - 3.5f)), label);
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + text_offset_x + 7.f, pos.y + style.FramePadding.y);
        TextEx(label, label_display_end);
    }
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(selected ? ImGuiCol_ButtonHovered : ImGuiCol_Text));
