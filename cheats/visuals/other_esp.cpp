...
#include "..\misc\logs.h"
#include "..\misc\misc.h"
#include "../animation-system/local/local_animations.h"
...
void otheresp::penetration_reticle()
{
	...
	// no need to draw this on knife or grenade.
...
void otheresp::automatic_peek_indicator()
{
	...
	if (!weapon->is_non_aim() && key_binds::get().get_key_bind_state(18) || alpha)
	{
		...
		render::get().Draw3DFilledCircle(position, alpha * 15.f, g_ctx.globals.fired_shot ? Color(106, 153, 249, (int)(alpha * 135.0f)) : Color(106, 153, 249, (int)(alpha * 125.0f)));
