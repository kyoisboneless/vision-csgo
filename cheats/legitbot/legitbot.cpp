...
float static_fov_to(const Vector& viewAngle, const Vector& aimAngle) {
	...
	math::normalize_angles(delta);
	return math::fast_sqrt(powf(delta.x, 2.0f) + powf(delta.y, 2.0f));
}
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	// calculate field of view on target.
	calc_fov();
	...
	// are we flashed?
	if (!g_cfg.legitbot.do_if_local_flashed && g_ctx.local()->m_flFlashDuration() >= 0.000001f)
	{
		autofire_delay = 0;
