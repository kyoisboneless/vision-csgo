...
{
	if (g_ctx.globals.updating_animation && m_clientstate()->m_nDeltaTick != -1)
...
_declspec(noinline)void hooks::buildtransformations_detour(player_t* player, studiohdr_t* hdr, Vector* pos, Quaternion* q, const matrix3x4_t& cameraTransform, int boneMask, byte* boneComputed) {
	// get bone jiggle.
	...
	*reinterpret_cast<int*>(uintptr_t(player) + 0x292C) = 0;
	...
	// call og.
	((BuildTransformationsFn)original_buildtransformations)(player, hdr, pos, q, cameraTransform, boneMask, boneComputed);

	// restore bone jiggle.
	*reinterpret_cast<int*>(uintptr_t(player) + 0x292C) = bone_jiggle;
}

void __fastcall hooks::hooked_buildtransformations(player_t* player, void* edx, studiohdr_t* hdr, Vector* pos, Quaternion* q, const matrix3x4_t& cameraTransform, int boneMask, byte* boneComputed) {
	return buildtransformations_detour(player, hdr, pos, q, cameraTransform, boneMask, boneComputed);
}

int hooks::processinterpolatedlist() {
	static auto allow_extrapolation = *(bool**)(util::FindSignature(crypt_str("client.dll"), crypt_str("A2 ? ? ? ? 8B 45 E8 F3")) + 0x1);
