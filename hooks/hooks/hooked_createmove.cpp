...
#include "..\..\cheats\slow-walk\slowwalk.h"
...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	c_movement::get().auto_jump();

	misc::get().SlideWalk(m_pcmd);

	misc::get().NoDuck(m_pcmd);

	c_movement::get().fake_duck(m_pcmd);
	...
	c_movement::get().auto_strafe(m_pcmd);
	...
	c_movement::get().automatic_peek(m_pcmd, wish_angle.y);
	...
	c_movement::get().movement_fix(wish_angle, m_pcmd);
	...
	//	hooks::netchannel_hook = new vmthook(reinterpret_cast<DWORD**>(net_channel));
	//	hooks::netchannel_hook->hook_function(reinterpret_cast<uintptr_t>(hooks::process_packet), 39);
	}
