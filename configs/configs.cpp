...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.position_adjustment, false, crypt_str("Ragebot.position_adjustment"));
	...
	setup_item(&g_cfg.esp.viewmodel_changer, false, crypt_str("Esp.viewmodel_changer"));
	...
	setup_item(&g_cfg.misc.auto_accept, false, crypt_str("Misc.auto_accept"));
	...
	setup_item(&g_cfg.menu.main, Color(66, 135, 245), crypt_str("Menu.main_theme"));
	setup_item(&g_cfg.menu.gradients, Color(83, 121, 181), crypt_str("Menu.gradient_start"));
	setup_item(&g_cfg.menu.gradiente, Color(108, 72, 156), crypt_str("Menu.gradient_end"));
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
		{
			...
			file = std::string(path) + crypt_str("\\vision\\configs\\") + config;
		}
